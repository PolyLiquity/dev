...
contract PLQTYToken is CheckContract, IPLQTYToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "PLQTY";
    string constant internal _SYMBOL = "PLQTY";
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'PLQTY: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'PLQTY: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "PLQTY: Cannot transfer tokens directly to the LQTY token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != lqtyStakingAddress,
            "PLQTY: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "PLUSDToken: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "PLUSDToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "PLUSDToken: caller must not be the multisig");
    }

    function _requireCallerIsLQTYStaking() internal view {
         require(msg.sender == lqtyStakingAddress, "PLUSDToken: caller must be the PLUSDStaking contract");
    }
